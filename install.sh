...
print_fail() { printf "%22s ${BOLD}${RED}[ FAIL ]${RESET}\n" ""; }
...
# 2. PRIVILEGE CHECK (Universal)
if [[ "$EUID" -ne 0 ]]; then
    printf "\n  ${RED}❌ Error:${RESET} Installation requires sudo privileges.\n"
    printf "     Please run: ${BOLD}sudo $0${RESET}\n\n"
    exit 1
fi
...
clear
printf "  ${BOLD}┌────────────────────────────────────────────────────────────────────┐${RESET}\n"
printf "  ${BOLD}│  ${CYAN}⚡ StackPulse${RESET}${BOLD} — Universal Installer v1.5%s│${RESET}\n" "$BOX_PADDING"
printf "  ${BOLD}└────────────────────────────────────────────────────────────────────┘${RESET}\n"
...
if install_deps; then
    print_done "(Docker $DOCKER_STATUS)"
else
    print_fail; exit 1
fi
...
sudo cp stackpulse.sh "$BINARY_PATH" && sudo chmod +x "$BINARY_PATH"
print_done


# [4/5] Background Service & Timer
...
if [[ "$PLATFORM" == "linux" ]]; then
    ...
    # macOS Protection: Kill rogue root processes and restart as User
    if pgrep -u root nginx >/dev/null 2>&1; then
        sudo pkill -u root nginx >/dev/null 2>&1
    fi
    if command -v brew &>/dev/null; then
        sudo -u "$REAL_USER" brew services restart nginx >/dev/null 2>&1
        print_done
    else
        printf "${YELLOW}[ SKIPPED ]${RESET}\n"
    fi
